...
<section>
    <div class="container" id="con_bottom2" style="margin-left: 0px;">
        <div>
            <div align="center">
                <a role="button">
                    <video id="videoPlay_2" width="14%" src="{{url($listen_spell->ls_pro)}}"
    ...
    <div class="container">
        @if($article['pre'])
            <a href="{{url('listen_spell_all/'.$topic->top_id.'_'.$article['pre']["ls_id"])}}" target="_top"
               class="btn btn-primary"
               id="word_left2"
               role="button"><span class="glyphicon glyphicon-arrow-left" style="color: #666;">　</span>previous</a>
        @else

            <a href="{{url('listen_spell_all/'.$topic->top_id.'_'.$listen_spell->ls_id)}}" target="_top"
               class="btn btn-primary"
               id="word_left2"
               role="button">None</a>
        @endif
        @if($article['next'])
            <a href="{{url('listen_spell_all/'.$topic->top_id.'_'.$article['next']->ls_id)}}" target="_top"
               class="btn btn-primary"
               id="word_right2"
    ...
    <div class="container" style="margin-top: 30px">
        ...
        <div class="col-md-12">
            ...
            @foreach($topic_all as $t)
                <a href="{{url('listen_spell_all/'.$t->top_id.'_'. $top_ls[$i]["ls_id"])}}" class="btn btn-primary"
...
<section>
    <div class="col-md-12" style="padding: 0;">
        <div class="container" style="padding: 0;">
            ...
            <div class="col-md-6">
                <a href="{{url('top_listen_spell/'.$get_text_id->text_id)}}" target="_top" class="btn btn-primary"
                   id="read_all"
                   role="button">返回列表</a>
            </div>
        </div>
    ...
    function check(flag) {
        if (flag == 1) {
            ...
            window.setInterval(function () {
                window.location.href = "{{url('listen_spell_all/'.$listen_spell->top_id.'_'.session('next'))}}";
            }, 1500);
...
<script>
    ...
    function result() {
        var result = '{{$listen_spell->ls_name}}';
