...
<section>
    <div class="container" id="con_bottom2" style="margin-left: 0px;">
        <div>
            <h3 class="ws_name">{{$watch_spell->ws_mean}}</h3>
            <h3 class="ws_name2">{{$watch_spell->ws_name}}</h3>
        </div>
        <div>
            @if(session('right')==0)
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose2" role="button"
                   onclick="check(1)">{{$watch_spell->ws_right}}</a>
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose2" role="button"
                   onclick="check(0)">{{$watch_spell->ws_error1}}</a>
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose2" role="button"
                   onclick="check(0)">{{$watch_spell->ws_error2}}</a>
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose2" role="button"
                   onclick="check(0)">{{$watch_spell->ws_error3}}</a>
    ...
    <div class="container">
        @if($article['pre'])
            <a href="{{url('watch_spell_all/'.$topic->top_id.'_'.$article['pre']["ws_id"])}}" target="_top"
               class="btn btn-primary"
               id="word_left2"
               role="button"><span class="glyphicon glyphicon-arrow-left" style="color: #666;">　</span>previous</a>
        @else

            <a href="{{url('watch_spell_all/'.$topic->top_id.'_'.$watch_spell->ws_id)}}" target="_top"
               class="btn btn-primary"
               id="word_left2"
               role="button">None</a>
        @endif
        @if($article['next'])
            <a href="{{url('watch_spell_all/'.$topic->top_id.'_'.$article['next']->ws_id)}}" target="_top"
               class="btn btn-primary"
               id="word_right2"
    ...
    <div class="container" style="margin-top: 30px">
        ...
        <div class="col-md-12">
            ...
            @foreach($topic_all as $t)
                <a href="{{url('watch_spell_all/'.$t->top_id.'_'. $top_ws[$i]["ws_id"])}}" class="btn btn-primary"
...
<section>
    <div class="col-md-12" style="padding: 0;">
        <div class="container" style="padding: 0;">
            ...
            <div class="col-md-6">
                <a href="{{url('top_watch_spell/'.$get_text_id->text_id)}}" target="_top" class="btn btn-primary"
                   id="read_all"
                   role="button">返回列表</a>
            </div>
        </div>
    ...
    function check(flag) {
        if (flag == 1) {
            ...
            window.setInterval(function () {
                window.location.href = "{{url('watch_spell_all/'.$watch_spell->top_id.'_'.session('next'))}}";
            }, 1500);
