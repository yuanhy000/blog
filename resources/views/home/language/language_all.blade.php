...
<nav class="navbar">
    <div class="top_bg">
        <span>
            ...
            <a href="{{url('/language_conf')}}" class="btn btn-primary" id="conf_all" role="button" target="_top">
...
<section>
    <div class="container" id="con_bottom" style="margin-left: 0px;">
        <div>
            <img src="{{url($language->lan_thumb)}}" alt="" class="lan_thumb">
        </div>
        <div>
            @if(session('right')==0)
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose" role="button"
                   onclick="check(1)">{{$language->lan_right}}</a>
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose" role="button"
                   onclick="check(0)">{{$language->lan_error1}}</a>
                <a href="javascript:" target="_top" class="btn btn-primary" id="chose" role="button"
                   onclick="check(0)">{{$language->lan_error2}}</a>
    ...
    <div class="container">
        @if($article['pre'])
            <a href="{{url('language_all/'.$topic->top_id.'_'.$article['pre']["lan_id"])}}" target="_top"
               class="btn btn-primary"
               id="word_left"
               role="button"><span class="glyphicon glyphicon-arrow-left" style="color: #666;">　</span>previous</a>
        @else

            <a href="{{url('language_all/'.$topic->top_id.'_'.$language->lan_id)}}" target="_top"
               class="btn btn-primary"
               id="word_left"
               role="button">None</a>
        @endif
        @if($article['next'])
            <a href="{{url('language_all/'.$topic->top_id.'_'.$article['next']->lan_id)}}" target="_top"
               class="btn btn-primary"
               id="word_right"
    ...
    <div class="container" style="margin-top: 30px">
        ...
        <div class="col-md-12">
            ...
            @foreach($topic_all as $t)
                <a href="{{url('language_all/'.$t->top_id.'_'. $top_lan[$i]["lan_id"])}}" class="btn btn-primary"
...
<section>
    <div class="col-md-12" style="padding: 0;">
        <div class="container" style="padding: 0;">
            ...
            <div class="col-md-6">
                <a href="{{url('top_language/'.$get_text_id->text_id)}}" target="_top" class="btn btn-primary"
                   id="read_all"
                   role="button">返回列表</a>
            </div>
        </div>
    ...
    function check(flag) {
        if (flag == 1) {
            ...
            window.setInterval(function () {
                window.location.href = "{{url('language_all/'.$language->top_id.'_'.session('next'))}}";
            }, 1500);
