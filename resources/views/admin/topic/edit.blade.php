...
@section('content')
    ...
    <div class="result_wrap">
        <form action="{{url('admin/topic/'.$field->top_id)}}" method="post">
            ...
            <table class="add_tab">
                ...
                <tr>
                    ...
                    <td>
                        <select name="text_id">
                            @foreach($data as $d)
                                <option value="{{$d->text_id}}"
                                        @if($d->text_id == $field->text_id) selected @endif > <!--默认选中-->
                                    {{$d->text_name}}</option>
                            @endforeach
                        </select>
                    </td>
                ...
                <tr>
                    <th>章节名称：</th>
                    <td>
                        <input type="text" name="top_name" class="lg" value="{{$field->top_name}}">
                    </td>
                </tr>
                <tr>
                    <th>备注：</th>
                    <td>
                        <input type="text"  name="top_note" class="lg" value="{{$field->top_note}}">
                    </td>
                </tr>
                <tr>
                    <th>章节听力：</th>
                    <td>
                        <input type="text" size="50" name="top_listen" {{$field->top_listen}}>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <script id="editor" name="top_word" type="text/plain"
                                style="width:860px;height:500px;">{!! $field->top_word !!}</script>
                        <script type = "text/javascript" >
